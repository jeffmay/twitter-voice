...
object OAuth {
  ...
  def Authorizer(consumer: Consumer, token: Token): Authorizer = {
    ...
    new Authorizer {
      override def authorize(httpRequest: HttpRequest): HttpRequest = {
